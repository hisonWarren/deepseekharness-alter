...
/** Soft-refresh interval for already-loaded / expanded directories while visible. */
...
export function ExplorerView(props: {
  ...
  /** Fetch one directory; soft mode keeps prior entries until the new listing differs. */
  const refreshDir = useCallback((dir: string, soft = false) => {
    if (!soft) storeLevel(dir, {})
    api.fsTree({ sessionId, cwd }, dir).then((listing) => {
      const prev = dataRef.current[dir]
      if (soft && prev?.entries !== undefined && sameEntries(prev.entries, listing.entries)) return
  ...
  useEffect(() => {
    // Load the visible set; already-loaded levels stay cached until soft refresh
    // or the manual refresh button wipes the cache.
    const root = cwd
  ...
  useEffect(() => {
    ...
    const softRefreshVisible = (): void => {
      if (typeof document !== 'undefined' && document.visibilityState !== 'visible') return
      const targets = [cwd, ...expandedRef.current]
      for (const dir of targets) {
        const level = dataRef.current[dir]
        // Skip dirs still on the initial loading placeholder.
        if (level === undefined) continue
        if (level.entries === undefined && level.error === undefined) continue
        refreshDir(dir, true)
      }
    }
    const onFocus = (): void => { softRefreshVisible() }
    const onVisibility = (): void => {
      if (document.visibilityState === 'visible') softRefreshVisible()
    }
    window.addEventListener('focus', onFocus)
    document.addEventListener('visibilitychange', onVisibility)
    const timer = window.setInterval(softRefreshVisible, AUTO_REFRESH_MS)
    softRefreshVisible()
    return () => {
