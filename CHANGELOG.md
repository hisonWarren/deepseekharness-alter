...
### Fixed
- Outside-app pet: facing flip applies `scaleX` only to `<video>` (never stage/sprite), so bubble/badge text cannot mirror into 乱码 after 东张西望.

## 0.2.16 — 2026-08-16

### Added
- `dsh-ux-polish` 0.3.5→0.3.6: Ctrl+V / drop / paperclip materialize into `.dsh-inbox/` + `@` path (clipboard screenshots have no disk path).
- Docked **变动** vs better-sidebar: measure open sidebar width into `--dsh-ux-sidebar-w` and pad overlay so Explorer no longer covers the diff (width capped at min(520px, 42vw)).

### Fixed
- Outside-app pet bubble/badge text mirrored after 东张西望 (`scaleX` only on `#sprite`).
- Balance meter details contrast on light UI via ux-polish CSS (do not rewrite `dsh-balance-meter` client.js — PowerShell BOM corrupts the bundle).
- Docked 变动 chrome: restore left 12px radius + soft shadow; active tab borderless 8px peer session header.

## 0.2.15 — 2026-08-16

### Added
- `dsh-ux-polish` 0.3.4: paperclip **添加文件** accepts any local file into `.dsh-inbox/` / draft paths (~32MB). Host `POST /ux-polish/inbox`.
...
### Changed
- `dsh-ux-polish` 0.3.1: queue mode shows Queue闁?only (no companion gray Stop beside it); stock Stop returns when the draft is cleared.
...
### Changed
- `dsh-ux-polish`: peer-match session header chrome (borderless AI 閻庡厜鍓濋悡?aligned with 闁告瑦锚婵?/ open-editor; drop unnecessary 婵炴埊绠掓竟?outline).
...
### Changed
- In-app pet defaults to **bottom-left** (Explorer covers the right). Tray: 閹煎瓨姊婚弫銈夊礃閸涢偊鏀介悗?闁?鐎归潻缂氱粭鍛喆?/ 闁告瑥鍘栫粭鍛喆?
...
### Added
- Tray **闁煎浜滈悾鐐▕婢跺鏁╅柣?*: users can set their own proxy URL/port (not only Clash 7897).
